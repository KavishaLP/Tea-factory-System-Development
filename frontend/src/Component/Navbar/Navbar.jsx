...
const Navbar = ({ userName }) => {
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await axios.get('http://localhost:8081/api/manager/notifications/unread-count', {
    ...
    <nav className="navbar">
      ...
      <div className="navbar-right">
        <div className="notification-container">
          ...
          {showNotifications && <ManagerNotifications onClose={closeNotifications} />}
        </div>
        <div className="profile">
          <div className="profile-pic">
            <FaUser />
          </div>
          <span className="admin-name">{userName || 'Manager'}</span>
        </div>
      </div>
    </nav>
  );
