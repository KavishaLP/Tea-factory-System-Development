...
const [currentMonth, setCurrentMonth] = useState(new Date().getMonth());
  ...
  // Function to fetch monthly records
  const fetchMonthlyRecords = async (year, month) => {
    setMonthlyLoading(true);
    try {
      const response = await axios.post(
        'http://localhost:8081/api/manager/get-monthly-farmer-payments',
        { year, month },
        { withCredentials: true }
      );
      
      if (response.data.Status === 'Success') {
        setMonthlyRecords(response.data.payments);
      } else {
        setMonthlyRecords([]);
      }
    } catch (error) {
  ...
  const handleMonthChange = (direction) => {
    ...
    fetchMonthlyRecords(newYear, newMonth + 1); // Months are 1-based in backend
  };

  // Handle manual year/month selection
  const handleYearMonthChange = (e) => {
    ...
    fetchMonthlyRecords(
      name === 'year' ? parseInt(value) : currentYear,
      name === 'month' ? parseInt(value) + 1 : currentMonth + 1
    );
  };

  // Fetch monthly records when component mounts or tab changes
  useEffect(() => {
    if (activeTab === "toPayment") {
      fetchMonthlyRecords(currentYear, currentMonth + 1);
    }
