...
export const mngLogin = (req, res) => {
    ...
    const sql = isEmail
        ? "SELECT * FROM manageraccounts WHERE ADMINMAIL = ?" // Check for email
        : "SELECT * FROM manageraccounts WHERE username = ?"; // Check for username
...
export const mngForgotPassword = (req, res) => {
    ...
    const sql = "SELECT * FROM manageraccounts WHERE ADMINMAIL = ?";
    sqldb.query(sql, [email], (err, results) => {
        ...
        const updateSql = "UPDATE manageraccounts SET RESET_CODE = ?, RESET_EXPIRY = ? WHERE ADMINMAIL = ?";
...
export const mngCheckCode = (req, res) => {
    ...
    const sql = "SELECT * FROM manageraccounts WHERE ADMINMAIL = ? AND RESET_CODE = ?";
...
export const mngUpdatePassword = (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        const sql = "UPDATE manageraccounts SET PASSWORD = ? WHERE ADMINMAIL = ?";
        ...
        sqldb.query(sql, [hashedPassword, email], (err, results) => {
            ...
            const clearCodeSql = "UPDATE manageraccounts SET RESET_CODE = NULL, RESET_EXPIRY = NULL WHERE ADMINMAIL = ?";
